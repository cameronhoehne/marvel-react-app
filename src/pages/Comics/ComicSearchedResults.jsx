...
function ComicSearchedResults() {
    ...
    try {
        if (isNaN(input) === true) {
        const api = await fetch(`${url}?ts=${ts}&apikey=${pubKey}&hash=${privKey}&titleStartsWith=${input}&limit=${limit}`)
        ...
        if (data.data.results.length <= 0) {
            navigate("/noresults", {state: {location: "comics"}})
        }
    }
...
const makeDate = (number) => {
            let date = new Date(number.replace(/-/g, '\/').replace(/T.+/, ''));
...
const captureId = (event, id) => {
    ...
    window.scrollTo(0, 0)
    navigate("/category/comics/searched/" + id)
}
...
const Characters = () => {
    return (
        ...
        >
            <h2>Characters</h2>
             <ul>
                {result[0].characters.items.map((character, idx) => {
                    return (<m.li key={idx}
                            whileHover={{
                                scale: 1.1
                            }}
                            ...
                            >
                            <a onClick={(e) => extractId(character.resourceURI, "characters")} >{character.name}</a>
                            </m.li>)
                })}
             </ul>
             </InfoCard>
...
const Creators = () => {
    return (<InfoCard
        ...
        <h2>Creators</h2>
         <ul>
            {result[0].creators.items.map((creator, idx) => {
                return (<m.li key={idx}
                        whileHover={{
                            scale: 1.1
                        }}
                        ...
                        >
                        <a onClick={(e) => extractId(creator.resourceURI, "creators")} >{`${creator.name} - ${creator.role}`}</a>
                        </m.li>)
            })}
         </ul>
         </InfoCard>)
  ...
  return (
    ...
    <Container className='cover-image-comics'
    initial={{x: "100%"}} 
    animate={{x: "0%"}}
    transition={{duration: .5}}
    exit={{x: "0%"}}
    key={"comics-container"}
    >
        <m.h1
        initial={{y: "100%"}} 
        animate={{y: "0%"}}
        ...
        >Use the search bar below to find your favorite comics</m.p>
        <ComicSearch />
        ...
        (<div>
            <SingleResult/>
            <CardGrid>
            {result[0].characters.items.length > 0 ? 
            (<Characters/>
            )  :
            (<div></div>)}
            {result[0].creators.items.length > 0 ? 
            (<Creators/>
            )  :
            (<div></div>)}
        ...
        {(result.map((item, idx) => {
            return (
                ...
                <p>{`Released ${makeDate(item.dates[0].date)}`}</p>
                <p>{`On sale for $${item.prices[0].price}`}</p>
                </Card>
                </SLink>
                </m.div>
            )
...
export default ComicSearchedResults;
